...
function touchingFloor(sprite) {
    ...
    for (let i = 0; i < bricks.length; ++i) {
        if (sprite.colliding(bricks[i]) && sprite.y < bricks[i].y && sprite.x > bricks[i].x && sprite.x < bricks[i].x + bricks[i].w) return true;
    }

    for (let i = 0; i < badBricks.length; ++i) {
        if (sprite.colliding(badBricks[i])) return true;
    }
...
function setup() {
    ...
    badBricks.img = "/assets/images/sprites/box.png";
...
function drawFrame() {
    ...
    camera.zoomTo(map(abs(dist(sprite1.x, sprite1.x, sprite2.y, sprite2.y)), 800, 0, 2, 1), 1000);
